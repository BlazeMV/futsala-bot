...
class OutCallbackQuery extends Command
{
    protected $name = "out";

    protected $description = "Out button Clicked";
    ...
    public function handle($arguments)
    {
        ...
        if ($event->step !== "join_time") {
            Telegram::answerCallbackQuery([
                'callback_query_id' => $cbq->getId(),
                'text' => "Sorry! leaving time for this event is over!"
            ]);
            return;
        }
        ...
        if (!$isIn) {
            ...
            $res = Telegram::answerCallbackQuery([
                'callback_query_id' => $cbq->getId(),
                'text' => "You haven't joined this event yet!"
            ]);
            //dd($res);
            return;
        }

        $participant = Participant::where('user_id', $cbq_sender->getId())->first();
        $participant->delete();
